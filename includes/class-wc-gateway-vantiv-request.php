<?php
/**
 * Class WC_Gateway_Vantiv_Request file.
 */
...
/**
 * Generates requests to send to Vantiv.
 */
class WC_Gateway_Vantiv_Request {

	/**
	 * Stores line items to send to Vantiv.
	 *
	 * @var array
	 */
	 ...
	 *
	 * @var WC_Gateway_Vantiv
	 */
	protected $gateway;

	/**
	 * Endpoint for requests from Vantiv.
	 *
	 * @var string
	 */
	...
	/**
	 * Endpoint for requests to Vantiv.
	 *
	 * @var string
	 */
	protected $endpoint;
	 ...
	 * Constructor.
	 *
	 * @param WC_Gateway_Vantiv $gateway Vantiv gateway object.
	 */
	public function __construct( $gateway ) {
		$this->gateway    = $gateway;
		$this->notify_url = WC()->api_request_url( 'WC_Gateway_Vantiv' );
	}

	/**
	 * Get the Vantiv request URL for an order.
	 *
	...
	public function get_request_url( $order, $sandbox = false ) {
		$this->endpoint    = 'https://certtransaction.hostedpayments.com/';
		$vantiv_args       = $this->get_vantiv_args( $order );
		$vantiv_args['bn'] = 'WooThemes_Cart'; // Append WooCommerce Vantiv Partner Attribution ID. This should not be overridden for this gateway.
		...
		WC_Gateway_Vantiv::log( 'Vantiv Request Args for order ' . $order->get_order_number() . ': ' . wc_print_r( array_merge( $vantiv_args, array_intersect_key( $mask, $vantiv_args ) ), true ) );
	...
	/**
	 * Get transaction args for vantiv request, except for line item args.
	 *
	 * @param WC_Order $order Order object.
	 * @return array
	 */
	protected function get_transaction_args( $order ) {
		$arr = array_merge(
			array(
				...
				'state'         => $this->get_vantiv_state( $order->get_billing_country(), $order->get_billing_state() ),
	 ...
	 *
	 * If URL is longer than 2,083 chars, ignore line items and send cart to Vantiv as a single item.
	 ...
	 *
	 * @param WC_Order $order Order to be sent to Vantiv.
	 * @param array    $vantiv_args Arguments sent to Vantiv in the request.
	 * @return array
	 */
	protected function fix_request_length( $order, $vantiv_args ) {
		$max_vantiv_length = 2083;
		$query_candidate   = http_build_query( $vantiv_args, '', '&' );

		if ( strlen( $this->endpoint . $query_candidate ) <= $max_vantiv_length ) {
			return $vantiv_args;
		}

		return apply_filters(
			'woocommerce_vantiv_args',
			array_merge(
	...
	/**
	 * Get Vantiv Args for passing to PP.
	 *
	 * @param  WC_Order $order Order object.
	 * @return array
	 */
	protected function get_vantiv_args( $order ) {
		WC_Gateway_Vantiv::log( 'Generating payment form for order ' . $order->get_order_number() . '. Notify URL: ' . $this->notify_url );

//		$force_one_line_item = apply_filters( 'woocommerce_vantiv_force_one_line_item', false, $order );
//
//		if ( ( wc_tax_enabled() && wc_prices_include_tax() ) || ! $this->line_items_valid( $order ) ) {
//			$force_one_line_item = true;
//		}
//
//		$vantiv_args = apply_filters(
//			'woocommerce_vantiv_args',
//			array_merge(
//				$this->get_transaction_args( $order ),
...
//
//	/**
//	 * Get phone number args for vantiv request.
//	 *
//	 * @param  WC_Order $order Order object.
//	 * @return array
//	 */
...
//
//	/**
//	 * Get shipping args for vantiv request.
//	 *
//	 * @param  WC_Order $order Order object.
//	 * @return array
//	 */
...
//				$shipping_args['state']      = $this->get_vantiv_state( $order->get_shipping_country(), $order->get_shipping_state() );
...
//
//	/**
//	 * Get shipping cost line item args for vantiv request.
//	 *
//	 * @param  WC_Order $order Order object.
...
//		}
//
//		// Add shipping costs. Vantiv ignores anything over 5 digits (999.99 is the max).
//		// We also check that shipping is not the **only** cost as Vantiv won't allow payment
...
//
//	/**
//	 * Get line item args for vantiv request as a single line item.
//	 *
//	 * @param  WC_Order $order Order object.
//	 * @return array
//	 */
...
//
//	/**
//	 * Get line item args for vantiv request.
//	 *
//	 * @param  WC_Order $order Order object.
...
//			 *
//			 * For shipping, we longer use shipping_1 because vantiv ignores it if *any* shipping rules are within vantiv, and vantiv ignores anything over 5 digits (999.99 is the max).
//			 */
...
//		}
//
//		return apply_filters( 'woocommerce_vantiv_get_order_item_names', implode( ', ', $item_names ), $order );
//	}
//
//	/**
//	 * Get order item names as a string.
...
//		}
//
//		return apply_filters( 'woocommerce_vantiv_get_order_item_name', $item_name, $order, $item );
//	}
//
//	/**
...
//
//	/**
//	 * Check if the order has valid line items to use for Vantiv request.
//	 *
...
//
//	/**
//	 * Get line items to send to vantiv.
//	 *
//	 * @param  WC_Order $order Order object.
//	 */
...
//
//	/**
//	 * Get the state to send to vantiv.
//	 *
//	 * @param  string $cc Country two letter code.
//	 * @param  string $state State code.
//	 * @return string
//	 */
//	protected function get_vantiv_state( $cc, $state ) {
//		if ( 'US' === $cc ) {
//			return $state;
//		}
//
